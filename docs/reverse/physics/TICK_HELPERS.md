...
- Builds an impulse vector (collisionNormalY, collisionNormalX, spinTerm) each scaled by frictionFloor*impulseScale*a3 (spin term instead scaled by spinTerm*a3*0.25) and hands it to FUN_004ed3f0 then FUN_004adbf0 (rigid body layer, out of scope). The third argument a3 is not the substep time: both callers push the constant 1.0 (`push 3F800000h` before `call sub_498960` at the tick site just ahead of 0x49D0A5, and at 0x4A3082 in the PUSH branch of `car_respawn_state_machine`). The port passed the substep time (0.00143 s), which made the wall push 700 times too weak and let a car sink through the track border, read again 2026-09-24.
