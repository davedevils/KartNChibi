...
- State 1/2: each tick calls `respawn_checkpoint_nearest_on_list` (0x489890, renamed this pass) against the one list already chosen, comparing the new progress index against the stored one. If the index does not increase, and 0x9C5 (2501) ms passed since the last direction change (`uVar8 - DAT_01397378 < 0x9C5` at the equivalent test in state 2, 0x4A0B39), state escalates: 1 to 2 at 0x4A0AD8, 2 to 100 at 0x4A0B89 / 0x4A0C16.
- State 100: if the index resumes increasing, drop back to state 1 (0x4A0AF9-0x4A0B99 area). If it keeps not increasing and 0x7D0 (2000) ms pass (`EBP - lastTime > 0x7D0` test at 0x4A0C0A), commit: state resets to 0, current position is cached into car+0xA7878/0xA787C/0xA7880 (0x4A0C33-0x4A0C4A), car+0x6BC is set to 100 (0x4A0C52), the checkpoint name and timer block car+0x6C0..0x6E0 is zeroed (0x4A0C6F-0x4A0C8C).
